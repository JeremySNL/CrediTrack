...
describe("GET /prestamos/:id", () => {
  ...
  it("deberia devolver el prestamo solicitado", async () => {
    const res = await request(app)
      .get(`/prestamos/${prestamo.id}`)
  ...
  it("deberia rechazar un prestamo que no existe", async () => {
    const res = await request(app)
      .get("/prestamos/999999")
      .set("Authorization", `Bearer ${token}`);
  ...
  it("deberia rechazar peticion sin token", async () => {
    const res = await request(app).get(`/prestamos/${prestamo.id}`);
